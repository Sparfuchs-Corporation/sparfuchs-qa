...
async function evolveAgents(days = 30) {
  ...
  const snapshot = await db.collection(COLLECTIONS.CANARY_RUNS)
  ...
  const summary = runs.reduce((acc, run) => {
    run.results.forEach(r => {
      if (!acc[r.id]) acc[r.id] = { name: r.name, failures: 0, total: 0 };
      acc[r.id].total++;
      if (r.severity === 'HIGH') acc[r.id].failures++;
    });
    return acc;
  }, {} as any);
  ...
  if (process.env.QA_EVOLVE_DRY !== '1') {
    try {
      const output = execSync(`claude "${prompt.replace(/"/g, '\\"')}"`, { encoding: 'utf8' });
